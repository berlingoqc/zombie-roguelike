...
use crate::{game::entity::room::RoomComponent, generation::{entity::room::RoomConfig, LEVEL_PROPERTIES_SPAWN_NAME}};
    ...
    for level_event in level_events.read() {
        if matches!(level_event, LevelEvent::Spawned(_)) {
            for (entity, level_iid) in levels.iter() {
                ...
                commands.entity(entity).insert(RoomComponent{ config: room_config });
